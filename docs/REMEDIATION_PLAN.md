...
- [ ] **R-25 Implement RFC-aware conversation grouping.** Spec: [F58](./specs/F58-rfc-aware-conversation-grouping.md).
  ...
  - Evidence 2026-07-24: F58 adds bounded Message-ID/In-Reply-To/References parsing, mailbox-scoped opaque thread resolution, same-mailbox reply authorization, reply-aware autosaved drafts, durable Queue headers, Cloudflare/Resend header mapping, and HTML-first duplicate-quote suppression. `npm run verify` passes with 1,253 application tests at 100% coverage plus all 16 bridge tests. The focused Chromium reply contract passes; the full suite reached 39/40 before the existing Cloudflare dev-runtime teardown issue. Migration `0015` passes executable schema parity, is applied to production D1, and Wrangler reports no pending migrations. The OpenNext build and Wrangler dry run pass; Worker `c6e78f82-d416-438f-962c-32acb29299ac` is live with root HTTP 200 and unauthenticated auth HTTP 401. A controlled three-message chain remains required before checking R-25.
