...
- 2026-07-25, **corrupted production timestamp**: `routing_rules.rule_YohOjzpqyI6Ux6oI2CToY` (the LucidKith `*` rule) has `created_at = 1784768200000`, a millisecond value in a column Drizzle reads as seconds, so it renders as year 58527. The correct value is `1784768200` (2026-07-23T00:56:40Z). The schema's `$defaultFn(() => new Date())` writes seconds correctly, so this row was written by something that bypassed it — most plausibly direct SQL during the R-08 production catch-all migration. Impact is currently cosmetic because no view sorts or displays routing-rule creation time, but any future ordering by it would be wrong. Fix requires a one-row production `UPDATE`, and the wider lesson is that operator SQL bypasses schema conversions; consider an audit for millisecond values in other `{ mode: "timestamp" }` columns.
