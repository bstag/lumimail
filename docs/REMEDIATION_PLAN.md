...
- [ ] **R-11 Prevent orphaned raw inbound objects.** Spec: [F63](./specs/F63-r2-retention-and-cleanup.md).
  ...
  - Remaining: deploy, run the production report to size the backlog, delete it after operator approval, then enable `R2_SWEEP_ENABLED`. R-11 stays unchecked until every existing object has reached its intentional state.
