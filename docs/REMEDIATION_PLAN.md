...
- [ ] **R-18 Complete a production readiness exercise.** Depends on all earlier critical items.
  ...
  - **Remaining before the backup gate can be checked:** a full restore has never been performed against a live environment. The D1 restore was rehearsed into a throwaway database and the R2 restore path, though checksum-guarded and unit-tested, has never written to a bucket. Exercising this needs a spare database and bucket to restore into.
