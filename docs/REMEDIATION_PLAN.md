...
- [ ] **R-09 Implement actual external forwarding or remove the claim.** Spec: [F62](./specs/F62-external-forwarding.md).
  ...
  - Remaining: verification of a real destination address by its recipient, and one controlled forwarded message received at an external mailbox. R-09 stays unchecked until a forwarded message has actually been received, because nothing has yet exercised `message.forward()` against a live destination.
...
- [ ] **R-11 Prevent orphaned raw inbound objects.** Spec: [F63](./specs/F63-r2-retention-and-cleanup.md).
  ...
  - Remaining: run the owner-only report to size the backlog, delete it after operator approval, then set `R2_SWEEP_ENABLED=true` and redeploy. R-11 stays unchecked until every existing object has reached its intentional state; objects written before this release are still orphaned.
...
- [ ] **R-34 Make terminally failed outbound delivery recoverable.** Depends on R-10.
  ...
  - Remaining: one controlled production recovery of a genuinely failed message. The MVP recoverability gate stays unchecked until an operator performs it, because no production message has yet been recovered end to end.
