...
| F02 | Partial | F45 adds exact provider-backed apex/nested onboarding, explicit verify/enable actions, truthful DNS/status UI, production reconciliation, and automated/browser coverage; a controlled outbound send was confirmed | Domain administration still uses the general organization-member guard rather than an admin role. Cloudflare sending onboarding is preserved on Lumimail removal until ownership provenance is tracked. |
...
| F06 | Confirmed | API-key creation, hashing, bearer authentication, read/send scopes, one-time secret handling, lifecycle metadata, and owner-scoped permanent revocation are covered by API/auth/browser tests and F44 | Keys cannot be renamed, rescoped, restored, or automatically rotated; those behaviors are outside the shipped F44 scope. |
| F07 | Partial | F46 adds canonical `*` storage, matching `*@domain` input normalization, domain/tenant/target validation, alias → exact → local → mailbox → catch-all precedence, one catch-all per domain, same-zone preservation, safe Cloudflare catch-all GET/PUT lifecycle, actionable UI, 100% covered unit/API contracts, and two browser scenarios | Production provider inspection found `lucidkith.com` already forwards catch-all mail externally, so Lumimail correctly refuses automatic takeover. `henriksen.dev` is disabled/drop at the provider and still needs a Lumimail domain/mailbox plus controlled two-domain inbound acceptance. External forward/reject actions retain their separately tracked limitations. |
...
| F16 | Locally confirmed, production pending | F60 provisions/reuses exact Worker rules, stores only Lumimail-owned rule IDs, constrains source/target rows by organization, and supports cross-domain internal mailbox targets | External forwarding remains deliberately unavailable under R-09; migration and controlled production delivery remain. |
...
| F18 | Confirmed | Bounded RFC identity parsing, mailbox-scoped thread resolution, authorized reply sources, durable reply headers, and server-derived multipart quotations are implemented | Production thread `msg_s-nv0Beq22gB4SPkU5lLh` groups the inbound original, Lumimail reply, and external follow-up; bold/italic markup survives both nested quotation layers. |
...
| F26 | Confirmed as plain-text authoring with HTML-preserving replies | Message actions prefill compose routes; replies submit authored text plus an authorized source ID and derive multipart quotations server-side, while forwards retain editable plain-text quoting | Rich-text authoring, full MIME forwarding, and original-attachment forwarding are outside the current contract. |
...
| F30 | Locally confirmed, production pending | F60 adds explicit bounded mailbox membership, group create/update UI/API, one-query internal member resolution, and cross-domain fan-out | Production migration plus controlled delivery to at least two internal mailboxes remain; external members are out of R-26 scope. |
...
| F34 | Confirmed | A strict Workers-compatible allowlist sanitizes before storage, both message render paths apply browser defense in depth, and adversarial/fail-closed tests are included in the coverage gate | Verified in production with controlled reply and non-reply HTML messages. F59 safely re-sanitizes persisted HTML before outbound quotation; inline style/color remains intentionally excluded. |
...
| F05 claimed a dormant Tiptap/WYSIWYG authoring flow | The first 2026-07-28 reconciliation documented the active plain-text reality. The later MVP decision restores a constrained Tiptap editor to required scope with server-sanitized semantic HTML, a server-derived plain-text alternative, and the existing server-owned reply quotation boundary. |
