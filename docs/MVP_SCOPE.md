...
| F92 | Mantle presentation layer | In Progress | [F92](specs/F92-mantle-presentation-layer.md) | brand source archive, normalized sigils, semantic theme tokens, typography, PWA assets, application shells | Stages 1–3 and the source-fidelity correction are staging-proven: the live mark and versioned PWA assets preserve the supplied sigil geometry, with semantic colors, typography, and application shells applied. Exact-artifact production promotion remains. Behavior, data, authorization, routes, and infrastructure remain unchanged. |
