...
| F83 | Access and security center | In Progress | [F83](specs/F83-access-and-security-center.md) | `/members`, `/api/admin/access-overview` | Deployed first read-only slice shows organization role separately from explicit mailbox roles and effective read/send/manage capabilities, including zero-access and zero-assignment states. Tenant-negative contracts, 390px UI, public smoke, anonymous denial, and remote doctor pass. Bulk grants, invitation delivery state, sessions, and audit history remain later slices. |
