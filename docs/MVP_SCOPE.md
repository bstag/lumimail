...
| F12 | Multi-user workspace: organizations, invites, roles, mailbox access | In Progress | [F12](specs/F12-multi-user-workspace.md), [F47](specs/F47-mailbox-access-control.md) | `/members`, `/api/org/members*`, mailbox-scoped APIs | Least-privilege viewer/responder/manager mailbox ACLs are deployed and await a controlled live multi-user flow; invites are not emailed and registration does not enforce the invited address. |
