...
| F82 | Read-only Operations Center | In Progress | [F82](specs/F82-operations-center.md) | `/operations`, `/api/admin/operations` | First owner-only slice is deployed and aggregates sanitized application/schema, persisted queue, and R2-retention status. Production smoke, anonymous refusal, version/binding readiness, and zero pending migrations pass; authenticated rendering is production-shaped locally. Binding detail, recovery, release, smoke, mail-flow, and cron evidence remain later post-MVP slices. |
