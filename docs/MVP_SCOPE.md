...
| F79 | Remote recovery rehearsal | In Progress | [F79](specs/F79-remote-recovery-rehearsal.md) | recovery scripts and operator runbook | Capture, remote restore, hashes/schema, public smoke, authenticated message/attachment reads, rich-HTML UI, and direct cross-mailbox denial now pass. The verifier and session were removed cleanly. At-rest archive policy, Worker rollback, resource cleanup, and final runbook remain. |
...
| P2 | Finish rollback and cleanup against spare remote resources | Remote restore, exact object checks, isolated Worker, authenticated app reads, and mailbox isolation pass; Worker rollback, resource cleanup, and archive policy remain. | [R-18](REMEDIATION_PLAN.md#phase-5--operational-hardening) |
