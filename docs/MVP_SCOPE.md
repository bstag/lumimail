...
| P2 | Finish cleanup against spare remote resources | The recovery Worker is removed; guarded cleanup supports that exact state, and an exact-target read-only recapture is ready. Choose an encrypted destination, run/verify recapture, then delete D1/R2 through the guard. Archive retention policy also remains. | [R-18](REMEDIATION_PLAN.md#phase-5--operational-hardening) |
