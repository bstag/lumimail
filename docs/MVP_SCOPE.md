...
| F82 | Read-only Operations Center | In Progress | [F82](specs/F82-operations-center.md) | `/operations`, `/api/admin/operations` | First owner-only slice aggregates sanitized application/schema, persisted queue, and R2-retention status. Binding, recovery, release, smoke, mail-flow, and cron evidence remain later post-MVP slices. |
