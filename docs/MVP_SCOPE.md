...
| F51 | Restricted-member administration navigation | Specified | [F51](specs/F51-restricted-member-admin-navigation.md) | mailbox selector, `(admin)` layout, `/api/auth/me` | Server guards correctly reject members with `403`; client navigation, direct-route redirects, and forbidden-control visibility remain to implement. |
