...
| P2 | Finish cleanup against spare remote resources | The recovery Worker is removed; the guarded command now supports that exact resume state, but D1/R2 deletion remains fail-closed until the private backup path is available for re-verification. Archive retention/encryption policy also remains. | [R-18](REMEDIATION_PLAN.md#phase-5--operational-hardening) |
