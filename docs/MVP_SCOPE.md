...
| F03 | Organization-scoped mailbox CRUD | Implemented Locally | [F03](specs/F03-mailboxes.md), [F47](specs/F47-mailbox-access-control.md) | `/mailboxes`, `/api/admin/mailboxes`, `/api/mailboxes*` | Organization admins provision and delete mailboxes; content/settings access requires explicit mailbox membership. Migration and production validation are pending. |
...
| F12 | Multi-user workspace: organizations, invites, roles, mailbox access | In Progress | [F12](specs/F12-multi-user-workspace.md), [F47](specs/F47-mailbox-access-control.md) | `/members`, `/api/org/members*`, mailbox-scoped APIs | Least-privilege viewer/responder/manager mailbox ACLs are implemented locally and await migration/deployment validation; invites are not emailed and registration does not enforce the invited address. |
