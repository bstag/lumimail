...
| F79 | Remote recovery rehearsal | In Progress | [F79](specs/F79-remote-recovery-rehearsal.md) | recovery scripts and operator runbook | A clean deployed commit now has a read-only production D1/R2 capture with canonical v1 evidence, restricted local ACLs, and a second zero-problem offline verification. At-rest archive policy, isolated remote restore, app verification, rollback, and cleanup remain. |
