...
| F12 | Multi-user workspace: organizations, invites, roles, mailbox access | In Progress | [F12](specs/F12-multi-user-workspace.md), [F47](specs/F47-mailbox-access-control.md), [F48](specs/F48-role-aware-mail-actions-and-shared-draft-refresh.md), [F49](specs/F49-identity-bound-organization-invitations.md), [F50](specs/F50-account-switch-cache-isolation.md) | `/members`, `/api/org/members*`, mailbox-scoped APIs | Least-privilege mailbox ACLs, role-aware actions, draft privacy, revocation, shared-draft refresh, identity-bound invitation acceptance, explicit mailbox assignment, restricted invited-user login, and cross-account browser-cache isolation are production-verified. Invitations remain copy-link rather than emailed, and restricted organization members still see client navigation for server-forbidden administration pages. |
| F51 | Restricted-member administration navigation | Implemented Locally | [F51](specs/F51-restricted-member-admin-navigation.md) | mailbox selector, `(admin)` layout, `/api/auth/me` | Members fail closed without the admin selector entry and direct admin routes redirect before controls render; owner/admin access remains. Deployment and controlled production validation remain. |
