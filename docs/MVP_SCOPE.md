...
| F85 | Unified settings shell | Shipped | [F85](specs/F85-unified-settings-shell.md) | `/settings`, `/settings/api-keys`, guarded admin/security/operations routes | Personal, mailbox, integrations, organization, security, operations, and update evidence are presented as one role-filtered lifecycle. Existing routes and server authorization remain authoritative; no update/deploy mutation is added. |
| F86 | Desktop split view and conversation rows | In Progress | [F86](specs/F86-desktop-split-view.md) | folder lists, message detail routes, `/api/messages` | Specification fixes URL/history, mobile fallback, bounded resizing, thread-count enrichment, accessibility, and isolation contracts before implementation. |
