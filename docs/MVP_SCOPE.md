...
| F83 | Access and security center | In Progress | [F83](specs/F83-access-and-security-center.md) | `/members`, `/api/admin/access-overview`, `/api/admin/sessions`, `/api/auth/reconfirm` | Deployed access matrix separates organization role from explicit mailbox capabilities. Deployed owner-only session visibility adds identity, issue/expiry time, and current-session marking without credential or invented device metadata. The exact-session recent-authentication foundation adds a 15-minute password proof, durable attempt limiting, and migration `0029`; destructive controls and their UI remain later slices. |
