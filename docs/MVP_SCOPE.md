...
| F79 | Remote recovery rehearsal | In Progress | [F79](specs/F79-remote-recovery-rehearsal.md) | recovery scripts and operator runbook | Read-only production capture and guarded isolated remote restore now pass: 29 table counts/migrations match, foreign keys are clean, 15 R2 hashes match, and the recovery Worker passes public smoke with no mail/async bindings. At-rest archive policy, authenticated isolation/app verification, rollback, and cleanup remain. |
...
| P2 | Finish recovery verification and rollback against spare remote resources | Remote D1/R2 restore, exact object checks, isolated Worker deployment, and public smoke pass; authenticated application/isolation checks, Worker rollback, cleanup, and archive policy remain. | [R-18](REMEDIATION_PLAN.md#phase-5--operational-hardening) |
