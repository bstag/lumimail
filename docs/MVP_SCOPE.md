...
| F12 | Multi-user workspace: organizations, invites, roles, mailbox access | In Progress | [F12](specs/F12-multi-user-workspace.md), [F47](specs/F47-mailbox-access-control.md), [F48](specs/F48-role-aware-mail-actions-and-shared-draft-refresh.md) | `/members`, `/api/org/members*`, mailbox-scoped APIs | Least-privilege mailbox ACLs are deployed; role-aware mail actions and draft privacy are implemented locally pending deployment. Invites are not emailed and registration does not enforce the invited address. |
