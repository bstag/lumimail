...
| F04 | Mail folders: inbox, sent, drafts, archive, spam, trash, starred | Shipped | [F04](specs/F04-mail-folders.md), [F72](specs/F72-mail-ui-state-synchronization.md) | dashboard folders, `/api/messages*` | Shared mutation invalidation keeps folder rows, filtered membership, detail controls, drafts, and navigation counts synchronized. Archive gained a read path on 2026-07-31. Pagination validation, scoped label subqueries, and bounded thread-count batches were deployed 2026-09-06; smoke 8/8 and remote doctor 26/26 pass. |
...
| F06 | API keys | Shipped | [F06](specs/F06-api-keys.md), [F44](specs/F44-api-key-lifecycle.md) | `/api-keys`, `/api/api-keys`, `/api/v1/send` | Keys are created with a one-time secret, lifecycle metadata is visible, and owner-scoped permanent revocation is enforced during authentication. Browser/API/SMTP sends share the same durable per-user quota at the sending interface (deployed 2026-09-06). |
...
| F08 | Webhooks | Shipped | [F08](specs/F08-webhooks.md) | `/webhooks`, `/api/webhooks*`, scheduled delivery jobs | External POSTs run outside mail consumers with bounded concurrency, five-second deadlines, and three attempts. Migrations 0040–0041 and the fix are deployed 2026-09-06; smoke 8/8 and remote doctor 26/26 pass. Controlled delivery/load evidence remains pending. Payload/privacy behavior remains part of the data-export audit. |
...
| F89 | External mail accounts and unified mailbox | Planned | [F89](specs/F89-external-mail-accounts.md), [F93](specs/F93-external-sync-bounded-memory.md) | `/settings/external-accounts`, `/api/external-accounts*`, `EXTERNAL_SYNC_QUEUE` | OAuth-first Google/Microsoft aggregation is implemented with bounded import, provider sending, lifecycle controls, and prospective retained originals. The F93 memory fix is deployed 2026-09-06 with smoke 8/8 and doctor 26/26 passing. A 2026-09-23 callback fix accepts provider-appended OAuth parameters and shows connection results on the settings page; it was deployed 2026-09-23 (version 5f756117). The feature remains Planned until controlled provider evidence is recorded; generic IMAP/SMTP, full-history verified backup, and two-way synchronization are later stages. |
...
| F92 | Mantle presentation layer | Shipped | [F92](specs/F92-mantle-presentation-layer.md) | brand source archive, normalized sigils, semantic theme tokens, typography, PWA assets, application shells | The staged identity, source-faithful mark and versioned PWA assets, semantic colors, typography, and application shells are production-verified. Behavior, data, authorization, routes, and infrastructure remain unchanged. |
...
specifications: F10 and F14–F32. Those features need specs defining their
