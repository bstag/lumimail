...
| F12 | Multi-user workspace: organizations, invites, roles, mailbox access | In Progress | [F12](specs/F12-multi-user-workspace.md), [F47](specs/F47-mailbox-access-control.md), [F48](specs/F48-role-aware-mail-actions-and-shared-draft-refresh.md), [F49](specs/F49-identity-bound-organization-invitations.md) | `/members`, `/api/org/members*`, mailbox-scoped APIs | Least-privilege mailbox ACLs, role-aware actions, and draft privacy are deployed pending controlled live validation. Identity-bound, mailbox-free invite acceptance is implemented locally pending deployment; invitations remain copy-link rather than emailed. |
