...
| F53 | Theme token consistency (light + dark) | In Progress | [F53](specs/F53-theme-token-consistency.md) | `src/app/globals.css`, all pages/components | Hardcoded palette utilities were replaced with semantic tokens and local verification passes; deliberate light/dark production visual validation remains pending. |
