...
| F82 | Read-only Operations Center | In Progress | [F82](specs/F82-operations-center.md) | `/operations`, `/api/admin/operations`, `/api/admin/operations/evidence` | Deployed overview aggregates sanitized application/schema, queue, R2-retention, and category-only runtime binding/provider presence without identifiers or secrets. A locally verified, organization-scoped, recently-authenticated content-free ledger now accepts and renders recovery, release, smoke, and mail-flow result history; deployment and producer adapters remain. Live Cron evidence remains delegated to the provider doctor. |
