...
| F54 | Durable outbound delivery | In Progress | [F54](specs/F54-durable-outbound-delivery.md) | `/api/send`, `/api/v1/send`, `OUTBOUND_QUEUE`, Sent UI | Queue producer/consumer, idempotent claim, classified retries, DLQ finalization, and queued/failed UI pass locally; production migration, deployment, and traced retry/no-duplicate validation remain. |
