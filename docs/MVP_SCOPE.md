...
| F81 | Signed releases and deliberate promotion | In Progress | [F81](specs/F81-signed-releases.md) | release manifest/signature tooling, `scripts/release-keygen.mjs`, `scripts/release-promote.mjs` | Offline trust chain passes provenance 18, archives 11, bundles 5, checkout derivation 11, preparation 5, signing 4, pinned verification 5, key custody 20, and verified promotion 15 contracts. Key authority is settled as one offline operator key for this deployment only. Promotion refuses unless trust, clean-checkout commit equality, and exact artifact digest all bind, then uploads without traffic, smokes that version, and promotes separately. No key has been generated and no signed release has been promoted yet. |
| F82 | Read-only Operations Center | In Progress | [F82](specs/F82-operations-center.md) | `/operations`, `/api/admin/operations`, `/api/admin/operations/evidence*` | The deployed overview and organization-scoped ledger are verified. Explicit authenticated adapters derive public-smoke, received mail-flow, signed-release, and recovery-archive results; none accepts arbitrary category/count/outcome input. Recovery evidence re-hashes the D1 export and every manifest object and states archive integrity only. Live Cron evidence moved to the F80 doctor. Restore-rehearsal attestation, independent delivery attestation, and additional integrity observations remain later slices. |
