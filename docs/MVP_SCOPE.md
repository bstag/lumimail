...
| F87 | OAuth-protected MCP integration | In Progress | [F87](specs/F87-mcp-oauth.md) | `/mcp`, `/oauth/*`, `/settings/mcp`, `OAUTH_KV`, D1/Queues | Read-only and separately consented mail-action profiles, exact session/org/mailbox authorization, secret-free lifecycle/audit, bounded tools, and durable send idempotency pass local verification. Separate production/staging KV bindings exist, and isolated staging passes 11/11 managed action checks including real S256 PKCE, consent, exact-resource refusal, refresh replay rejection, idempotent send, and revocation. Production promotion and smoke remain. |
