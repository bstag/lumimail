...
| F05 | Plain-text compose, provider send, drafts, attachment UI | In Progress | [F05](specs/F05-compose-send.md), [F48](specs/F48-role-aware-mail-actions-and-shared-draft-refresh.md), [F55](specs/F55-outbound-attachment-delivery.md) | `/compose`, `/api/send`, `/api/drafts*`, `/api/v1/send` | Shared mailbox drafts are capability-scoped; atomic outbound attachment storage, queue loading, and Cloudflare/Resend provider encoding pass automated verification. Production recipient proof remains. Newly selected files are intentionally not persisted by draft autosave. |
...
| F55 | Outbound attachment delivery | In Progress | [F55](specs/F55-outbound-attachment-delivery.md) | compose, `/api/send`, `/api/v1/send`, R2, `OUTBOUND_QUEUE`, Cloudflare/Resend providers | Atomic acceptance, compensation, immutable metadata, exact-byte loading, and both provider adapters pass full local verification. Production external-recipient filename/type/byte validation remains. |
...
| F17 | Attachment storage, download, and metadata in R2 | Partially Shipped | [F55](specs/F55-outbound-attachment-delivery.md) | `/api/attachments*`, `/api/messages/[id]/attachments`, outbound queue | Outbound files now use durable R2 metadata/bytes and provider delivery; inbound MIME attachments are still never extracted. |
...
| P1 | Prove outbound attachment delivery in production | F55 replaces post-send upload with atomic R2/queue/provider delivery and passes local verification; a controlled recipient must still confirm exact filename, type, and bytes. | [R-20](REMEDIATION_PLAN.md#phase-2--sending-and-routing-correctness) — implementation complete 2026-07-24 |
