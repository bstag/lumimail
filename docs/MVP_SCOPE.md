...
| F18 | Conversation/thread view | In Progress | [F58](specs/F58-rfc-aware-conversation-grouping.md) | `/inbox/[id]`, `/api/messages/thread/[threadId]`, compose/drafts/send queues | RFC parsing, mailbox-scoped opaque grouping, authorized reply headers, draft propagation, and duplicate-quote rendering pass local verification; migration/deployment and a controlled production three-message chain remain required. |
