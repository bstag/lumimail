...
| F82 | Read-only Operations Center | In Progress | [F82](specs/F82-operations-center.md) | `/operations`, `/api/admin/operations` | Deployed first slice aggregates sanitized application/schema, queue, and R2-retention status; the next local slice adds category-only runtime binding/provider presence without identifiers or secrets. Recovery, release, smoke, mail-flow, and cron evidence remain later post-MVP slices. |
