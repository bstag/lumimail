...
| F80 | Operator lifecycle and readiness doctor | In Progress | [F80](specs/F80-operator-lifecycle.md) | `scripts/doctor.mjs`, operator CLI | Local checks pass 15/15 and live remote checks pass 10/10 across deployment, bindings, D1/R2/Queues, migrations, secret presence, Email Routing/Sending, and smoke. Provider-side Cron inventory remains an explicit warning. Post-MVP hardening. |
...
| F82 | Read-only Operations Center | In Progress | [F82](specs/F82-operations-center.md) | `/operations`, `/api/admin/operations` | Deployed overview aggregates sanitized application/schema, queue, R2-retention, and category-only runtime binding/provider presence without identifiers or secrets. Production deployment inventory, smoke, remote doctor, and unsigned-session redirect pass. Recovery, release, smoke/mail-flow evidence history, and live cron evidence remain later slices; the first three currently exist only as operator artifacts and need a content-free ingestion contract before the Worker can report them. |
| F83 | Access and security center | In Progress | [F83](specs/F83-access-and-security-center.md) | `/members`, `/api/admin/access-overview`, `/api/admin/sessions*`, `/api/admin/security-events`, `/api/admin/mailbox-grants`, `/api/auth/reconfirm` | The access matrix separates organization role from explicit mailbox capabilities. Owner-only session visibility and recent authentication support password-confirmed revoke controls with atomic content-free audit events; the bounded, tenant-scoped security history presents those events. Owners can add one member to as many as 25 mailboxes with an audited, additive, password-confirmed bulk grant that preserves existing access. Invitation delivery state remains a later slice. |
