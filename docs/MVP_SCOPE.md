...
| F89 | External mail accounts and unified mailbox | Planned | [F89](specs/F89-external-mail-accounts.md) | proposed `/settings/external-accounts`, `/api/external-accounts*`, `EXTERNAL_SYNC_QUEUE` | OAuth-first aggregation imports bounded Google/Microsoft mail copies and sends through the connected provider without mutating remote message state. Generic IMAP/SMTP, full-history verified backup, and two-way synchronization are explicit later stages. |
