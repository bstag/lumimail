...
| F88 | Privacy-preserving push notifications | In Progress | [F88](specs/F88-push-notifications.md) | `/api/push/*`, `/notifications/*`, `/settings/notifications`, service worker, D1/Queues | Implemented and locally verified with explicit per-device enrollment, mailbox preferences default off, exact-session and live mailbox authorization, opaque content-free payloads, isolated durable push queue/outbox, bounded retry/cleanup, lifecycle audit, and accessible Settings UI. Staging schema, VAPID secrets, queues, deploy, and public smoke pass; real subscribed-browser delivery/click/revocation evidence and production promotion remain. |
