...
| F12 | Multi-user workspace: organizations, invites, roles, mailbox access | In Progress | [F12](specs/F12-multi-user-workspace.md), [F47](specs/F47-mailbox-access-control.md), [F48](specs/F48-role-aware-mail-actions-and-shared-draft-refresh.md), [F49](specs/F49-identity-bound-organization-invitations.md), [F50](specs/F50-account-switch-cache-isolation.md) | `/members`, `/api/org/members*`, mailbox-scoped APIs | Least-privilege mailbox ACLs, role-aware actions, draft privacy, revocation, and shared-draft refresh are production-verified. F50 cross-account browser-cache isolation is implemented locally pending deployment and controlled validation before the F49 flow resumes; invitations remain copy-link rather than emailed. |
