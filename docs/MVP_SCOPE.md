...
| F83 | Access and security center | In Progress | [F83](specs/F83-access-and-security-center.md) | `/members`, `/api/admin/access-overview`, `/api/admin/sessions` | Deployed access matrix separates organization role from explicit mailbox capabilities. The next verified slice adds owner-only active-session identity, issue/expiry time, and current-session marking without credential or invented device metadata. Bulk grants, invitation delivery state, audited session revocation, and audit history remain later slices. |
