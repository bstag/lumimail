...
| F82 | Read-only Operations Center | In Progress | [F82](specs/F82-operations-center.md) | `/operations`, `/api/admin/operations`, `/api/admin/operations/evidence` | The deployed overview aggregates sanitized application/schema, queue, R2-retention, runtime readiness, and fixed recovery/release/smoke/mail-flow evidence states without identifiers or secrets. The organization-scoped ledger, recent-auth ingestion boundary, bounded retention, route denial, migration, and empty production state are verified. Authenticated producer adapters and live Cron evidence remain later slices. |
