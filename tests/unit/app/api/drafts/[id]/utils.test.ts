...
describe("selectDraftWithBody", () => {
	it("returns the draft when it belongs to the user and is a draft", async () => {
		...
		expect(await selectDraftWithBody(mock.db as never, "u1", "org_1", "msg_1")).toEqual(draft);
	});

	it("returns null when no draft row is found", async () => {
		mock.queueSelect([]);
		expect(await selectDraftWithBody(mock.db as never, "u1", null, "msg_1")).toBeNull();
	});

	it("returns null when the draft belongs to another user", async () => {
		mock.queueSelect([]);
		expect(await selectDraftWithBody(mock.db as never, "u1", "org_1", "msg_1")).toBeNull();
	});
